...
type_dec: type{strcpy($<t.str>$,$<t.str>1);}
...
char result[100];
sprintf(result,"ARRAY[OF][%s]%s",$<t.str>6,$<t.str>3);
strcpy($<t.str>$,result);
}
;
...
char result[100];
sprintf(result,"[RANGE->%s..%s][{:}]",$<t.str>1,$<t.str>3);
strcpy($<t.str>$,result);
}
;

const_value: num{strcpy($<t.str>$,$<t.str>1);}
|T_MINUS num{
char result[100];
...
cond_stmt: T_IF condition T_THEN block_stmt {
	char result[1000];
	sprintf(result, "[IF[COND%s][THEN_BODY%s]]", $<t.str>2,$<t.str>4);
	strcpy($<t.str>$, result);

}
| T_IF condition T_THEN block_stmt T_ELSE block_stmt {
	char result[1000];
	sprintf(result, "[IF[COND%s][THEN_BODY%s][ELSE_BODY%s]]", $<t.str>2,$<t.str>4,$<t.str>6);
	strcpy($<t.str>$, result);
...
{
        char result[1000];
	sprintf(result, "[FOR[COND[=[id:{%s}]%s][DOWNTO][=[id:{%s}]%s]][BODY%s]]",   						$<t.str>2,$<t.str>4,$<t.str>2,$<t.str>6,$<t.str>8);
	strcpy($<t.str>$, result);
...
{
   char result[1000];
	sprintf(result, "[FOR[COND[=[id:{%s}]%s][TO][=[id:{%s}]%s]][BODY%s]]",   						$<t.str>2,$<t.str>4,$<t.str>2,$<t.str>6,$<t.str>8);
	strcpy($<t.str>$, result);
}
;
